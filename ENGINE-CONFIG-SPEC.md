...
with that selection matches what was on screen. On a **multi-series** chart this accent is not applied — only the trigger label
...
with nothing to fill it stays literal in the rendered text — a chart without `columns.point_label`
or without a series column leaves `{point_label}` / `{series}` as written — and `{value}` works
alongside them.
...
e.g. `chart-scatter`), and `x-<xAxisType>` (every **chart** mount — `x-numeric`, `x-temporal`,
`x-quarterly`, or `x-categorical`, from `spec.xAxisType`; a table mount has no x axis and carries
`figure-card` alone). The x-axis-title breathing-room CSS
